...
def cleanse_trips(target_date: str, minio_key: str, minio_secret: str):
    spark = (
        SparkSession.builder
        .appName(f"rideflow_silver_trips_{target_date.replace('-', '')}")
    ...
    print(f"[cleanse_trips] Target Date: {target_date}")
    bronze_path = f"{BRONZE_BUCKET}/trips"

    try:
        df = spark.read.format("delta").load(bronze_path).filter(col("ingest_date") == target_date)
    except Exception as e:
        print(f"[cleanse_trips] No Bronze data found for {target_date}. Exiting.")
        return

    # 1. Deduplicate based on trip_id
    df = df.dropDuplicates(["trip_id"])
    initial_count = df.count()
    if initial_count == 0:
        print("[cleanse_trips] No records found.")
        return
    ...
    dq_cond = (
        ...
        col("status").isin(["completed", "cancelled", "no_driver"]) &
        col("city").isin(["HCMC", "HANOI"]) &
        (col("fare_vnd") >= 0) &
        (col("distance_km") >= 0.0)
    )
    ...
    print(f"[cleanse_trips] Evaluated {initial_count} records")
    print(f"[cleanse_trips] Valid: {valid_count} | Invalid (Quarantine): {invalid_count}")
...
if __name__ == "__main__":
    ...
    try:
        cleanse_trips(args.target_date, args.minio_key, args.minio_secret)
    except Exception as e:
